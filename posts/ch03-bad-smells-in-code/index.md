...
> 브라이언 푸트의 추측성 일반화(Brian Foote Speculative Generality)

- 현실에 미래 대비는 긍정적으로 다가 올 때도 있지만, 프로그래밍 세계에서는 다르다고 생각한다. 당장 필요없는 코드는 츠근대기 마련이다.
- **방법**
    - 계층 합치기(12.9)
    - 함수 인라인하기(6.2)
    - 클래스 인라인하기(6.7)
    - 함수 선언 바꾸기(6.5)
    - 죽은 코드 제거하기(8.9): 테스트 코드 외에는 사용하는 곳이 없는 함수와 클래스
...
- 캡슐화(encapsulation), 위임(delegation)
- **방법**
    - 중개자 제거하기(7.8): 실무 담당 객체와 직접 소통하기
    - 함수 인라인하기(6.2)
...
- 클래스에 필드가 과하면 중복이 발생하기 마련이다.
- ? 접두어, 접미어 필드는 추출 후보다.
- ? 클래스 안에서 자체적인 중복을 제거하는 방법
- **방법**
    - 클래스 추출하기(7.5)
    - 슈퍼클래스 추출하기(12.8)
    - 타입 코드를 서브클래스로 바꾸기(12.6)
...
- 데이터 클래스는 데이터 필드와 `getter`, `setter`로 구성된 클래스다.
- 데이터가 담겨있다보니 함부로 다뤄지는 경우가 많다.
- ? 불변 필드는 굳이 캡슐화할 필요 없고, 불변 데이터로부터 나온 결과는 게터를 통하지 않고 필드 자체를 공개해도 된다.
- **방법**
    - 레코드 캡슐화하기(7.1)
    - 세터 제거하기(11.7)
    - 함수 옮기기(8.1)
...
- **주석은 악취가 아닌 향기를 입힌다.** 당신은 주석을 탈취제로 사용하고 있는가?
- 과도한 주석은 악취를 풍기기 쉽다. 주석보다 의도를 어떻게 표현하여 전달할 것인지 먼저 강구하자.
- 무엇을 해야할 지 모를 때 사용하는 주석은 적절하다. 확실하지 않는 부분에 주석을 남기자.
- **방법**
    - 함수 추출하기(6.1)
    - 함수 선언 바꾸기(6.5)
    - 어서션 추가하기(10.6)
